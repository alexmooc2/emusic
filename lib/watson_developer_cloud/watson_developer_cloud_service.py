...
#         logging.info("url: " + str(full_url))
#         logging.info("jar: " + str(self.jar))
...
#         logging.info("files: " + str(files))
        ...
        response = urlfetch.fetch(method=method, url=full_url, headers=headers, payload = data, validate_certificate = True)
        if 200 <= response.status_code <= 299:
            if accept_json:
                response_json = json_import.loads(response.content)
